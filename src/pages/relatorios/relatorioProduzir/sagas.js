...
export function * fetch (api)  {
  try {    
      ...
      if (response.ok) {
        ...
        yield put(Action.relatorioProduzirSuccess(data))
      } else {
        const { message } = get(response, ['data'], {})
        yield put(Action.relatorioProduzirFailure(message))
     }
  } catch (ex) {
    yield put(Action.relatorioProduzirFailure())
  }
}

export function* imprimir(api, { obj }) {
  try {
    const response = yield call(api.RelatorioOrdemServico.imprimirProduzirProduto, obj)
    ...
    if (response.ok) {
      ...
      yield put(Action.relatorioProduzirSuccess({}))
    } else {
      const { message } = get(response, ['data'], {})
      yield put(Action.relatorioProduzirFailure(message))
    }
