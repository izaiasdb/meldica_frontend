...
export default class TabTelefone extends Component {
    ...
    adicionar = () => {
        ...
        let clienteTelefoneList = getFieldValue("cliente.clienteTelefoneList")

        if (telefone.id){
            let oldRegistro = clienteTelefoneList.find(c=> c.id == telefone.id)

            const index = clienteTelefoneList.indexOf(oldRegistro);

            if (index > -1) {
              clienteTelefoneList.splice(index, 1);
            }          
        }        

        clienteTelefoneList.push(telefone)
        this.limpar()
        setFieldsValue({
            cliente: { clienteTelefoneList },
            endereco: {
                id: null,
                //tipoTelefone: { id: 2},                
                idTipoTelefone: 1,
    ...
    remover = (index) => {
        const { form: {getFieldValue, setFieldsValue } } = this.props
        let clienteTelefoneList = getFieldValue("cliente.clienteTelefoneList")
        clienteTelefoneList = clienteTelefoneList.filter((e, indexx) => indexx != index)
        setFieldsValue({cliente: { clienteTelefoneList }})
        this.setStateView({ viewStateTab: INSERTING })
    }
    ...
    render() {
        ...
        const { clienteTelefoneList = [] } = cliente || {}    
        ...
        return (<div>
            ...
            <Card title={getTitle("Telefones")} style={{marginTop: '10px'}}>
                <Row gutter = { 12 }>
                    <Form.Item>
                        {
                            getFieldDecorator('cliente.clienteTelefoneList', {
                                initialValue: [...clienteTelefoneList],
