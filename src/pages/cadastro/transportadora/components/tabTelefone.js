...
export default class TabTelefone extends Component {
    ...
    adicionar = () => {
        ...
        let transportadoraTelefoneList = getFieldValue("transportadora.transportadoraTelefoneList")

        if (telefone.id){
            let oldRegistro = transportadoraTelefoneList.find(c=> c.id == telefone.id)

            const index = transportadoraTelefoneList.indexOf(oldRegistro);

            if (index > -1) {
              transportadoraTelefoneList.splice(index, 1);
            }          
        }        

        transportadoraTelefoneList.push(telefone)
        this.limpar()
        setFieldsValue({
            transportadora: { transportadoraTelefoneList },
            endereco: {
                id: null,
                //tipoTelefone: { id: 2},                
                idTipoTelefone: 1,
    ...
    remover = (index) => {
        const { form: {getFieldValue, setFieldsValue } } = this.props
        let transportadoraTelefoneList = getFieldValue("transportadora.transportadoraTelefoneList")
        transportadoraTelefoneList = transportadoraTelefoneList.filter((e, indexx) => indexx != index)
        setFieldsValue({transportadora: { transportadoraTelefoneList }})
        this.setStateView({ viewStateTab: INSERTING })
    }
    ...
    render() {
        ...
        const { transportadoraTelefoneList = [] } = transportadora || {}    
        ...
        return (<div>
            ...
            <Card title={getTitle("Telefones")} style={{marginTop: '10px'}}>
                <Row gutter = { 12 }>
                    <Form.Item>
                        {
                            getFieldDecorator('transportadora.transportadoraTelefoneList', {
                                initialValue: [...transportadoraTelefoneList],
