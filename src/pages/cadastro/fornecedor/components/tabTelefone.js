...
export default class TabTelefone extends Component {
    ...
    adicionar = () => {
        ...
        let fornecedorTelefoneList = getFieldValue("fornecedor.fornecedorTelefoneList")

        if (telefone.id){
            let oldRegistro = fornecedorTelefoneList.find(c=> c.id == telefone.id)

            const index = fornecedorTelefoneList.indexOf(oldRegistro);

            if (index > -1) {
              fornecedorTelefoneList.splice(index, 1);
            }          
        }        

        fornecedorTelefoneList.push(telefone)
        this.limpar()
        setFieldsValue({
            fornecedor: { fornecedorTelefoneList },
            endereco: {
                id: null,
                //tipoTelefone: { id: 2},                
                idTipoTelefone: 1,
    ...
    remover = (index) => {
        const { form: {getFieldValue, setFieldsValue } } = this.props
        let fornecedorTelefoneList = getFieldValue("fornecedor.fornecedorTelefoneList")
        fornecedorTelefoneList = fornecedorTelefoneList.filter((e, indexx) => indexx != index)
        setFieldsValue({fornecedor: { fornecedorTelefoneList }})
        this.setStateView({ viewStateTab: INSERTING })
    }
    ...
    render() {
        ...
        const { fornecedorTelefoneList = [] } = fornecedor || {}    
        ...
        return (<div>
            ...
            <Card title={getTitle("Telefones")} style={{marginTop: '10px'}}>
                <Row gutter = { 12 }>
                    <Form.Item>
                        {
                            getFieldDecorator('fornecedor.fornecedorTelefoneList', {
                                initialValue: [...fornecedorTelefoneList],
